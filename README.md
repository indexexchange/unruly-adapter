...
* `unruly-htb.js` - This is your partner module file, by default it contains a template divided into multiple sections which need to be completed.
* `unruly-htb-validator.js` - This is the validator file for the configuration object that will be passed into your module.
* `unruly-htb-exports.js` - A file that contains all of the modules exports (i.e. any functions that need to be exposed to the outside world).
...
1. <b>Complete the unruly-htb.js file </b>
    * unruly-htb.js is where all of your adapter code will live.
...
2. <b>Complete the unruly-htb-validator.js file</b>
...
3. <b>Complete the unruly-htb-exports.js file</b>
    * This file will contain any functions that need to be exported or exposed to the outside world. Things like render functions, custom callbacks, etc. Any legacy render functions will also need to be exposed here. Anything added to the `shellInterface.UnrulyHtb` will be accessible through `window.headertag.UnrulyHtb`
...
for (var i = 0; i < 10; i++) {
    ...
    if (w.headertag) {
        try {
            w.headertag.UnrulyHtb.render(document, %%PATTERN:TARGETINGMAP%%, '%%WIDTH%%', '%%HEIGHT%%');
            break;
        } catch (e) {
            continue;
        }
...
In this section you will be filling out the unruly-htb.js, unruly-htb-exports.js, and the unruly-htb-validator.js files to create your module.

### Step 0: Config Validation (`unruly-htb-validator.js`)
...
### Step 1: Partner Configuration (`unruly-htb.js`)
...
### Step 2: Generate Request URL (`unruly-htb.js`)
...
### Step 3: Response Callback (`unruly-htb.js`)
...
### Step 4: Parsing and Storing Demand (`unruly-htb.js`)
...
### Step 5: Rendering (`unruly-htb.js`)
...
### Step 6: Exports (`unruly-htb-exports.js`)
...
```javascript
shellInterface.UnrulyHtb = { //shell interface is the window variable that is accessable through the window object, currently this will always be window.headertag
    render: SpaceCamp.services.RenderService.renderDfpAd.bind(null, 'UnrulyHtb')
};
```
...
```javascript
if (__directInterface.Layers.PartnersLayer.Partners.UnrulyHtb) {
    shellInterface.UnrulyHtb = shellInterface.UnrulyHtb || {};
    shellInterface.UnrulyHtb.adResponseCallback = __directInterface.Layers.PartnersLayer.Partners.UnrulyHtb.adResponseCallback;
}
```
